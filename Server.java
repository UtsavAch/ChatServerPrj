...
public class Server
{
  ...
  static private boolean processInput( SocketChannel sc, Selector selector, SelectionKey key) throws IOException {
    // Clear buffer and read message
    buffer.clear();
    int bytesRead = sc.read(buffer);
    buffer.flip();

    // If no data was read, return false to close connection
    if (bytesRead == -1) {
      return false;
    }
    ...
    // Check if this is the first message from the client (nickname)
    if (key.attachment() == null) {
      // Attach the nickname to the key
      key.attach(message);
      System.out.println("User connected with nickname: " + message);
      return true;
    }

    // Get the nickname from the key attachment
    String nickname = (String) key.attachment();
    String fullMessage = nickname + ": " + message + "\n";

    System.out.print("Broadcasting: " + fullMessage);

    // Broadcast message to all active clients
    for (SelectionKey k : selector.keys()) {
      if (k.isValid() && k.channel() instanceof SocketChannel) {
        SocketChannel client = (SocketChannel) k.channel();
        ByteBuffer broadcastBuffer = encoder.encode(CharBuffer.wrap(fullMessage));
